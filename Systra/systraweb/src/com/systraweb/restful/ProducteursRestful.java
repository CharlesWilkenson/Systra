...
@Stateless
@Path("/producteur")
public class ProducteursRestful {
	...
	public Response addproducteur(@QueryParam(value="producteurId")String producteurId,
			@QueryParam(value="producteurNomComplet")String producteurNomComplet,
			@QueryParam(value="producteurSexe")String producteurSexe,
			@QueryParam(value="producteurTelephone")String producteurTelephone,
	...
	{
		String rep="";
		ProducteurEntity pro = new ProducteurEntity(producteurId, producteurNomComplet, producteurSexe, producteurTelephone, "Actif");
		
		ParcelleEntity par=new ParcelleEntity(parcelleId, pro, parcelleCommune ,
	...
	@GET
	@Path("/updateproducteur")
	@Produces(MediaType.APPLICATION_JSON)
	public Response addproducteur(@QueryParam(value="producteurId")String producteurId,
			@QueryParam(value="producteurNomComplet")String producteurNomComplet,
			@QueryParam(value="producteurSexe")String producteurSexe,
			@QueryParam(value="producteurTelephone")String producteurTelephone,
			@QueryParam(value="producteurEtat")String producteurEtat)
			
	{
		String rep="";
		ProducteurEntity pro = new ProducteurEntity(producteurId, producteurNomComplet, producteurSexe, producteurTelephone, producteurEtat);
		service.UpdateProducteur(pro);
		rep="success";
		System.out.print(producteurTelephone);
		return Response.ok()
				.header("Access-Control-Allow-Origin", "*")
				.header("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT")
				.entity(rep)
	...
	@GET
	@Path("findProducteur/{id}")
	@Produces(MediaType.APPLICATION_JSON)
	public ProducteurEntity findOne(@PathParam(value = "id")String id) {
	ProducteurEntity prod=	service.FindOneProducteur(id);
		return prod; 
	}
	...
	@Path("allproducteurs")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getProducteur() {
		ClassJsonOutput<ProducteurEntity> classJsonOuput = new ClassJsonOutput<ProducteurEntity>();
		List<ProducteurEntity> listprod=service.GetAllProducteur();
		classJsonOuput.setResults(listprod);
		return Response.ok()
				.header("Access-Control-Allow-Origin", "*")
				.header("Access-Control-Allow-Methods", "GET, POST, DELETE, PUT")
